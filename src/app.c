...
void app_init(App *app) {
	...
	app->screen_w = app->x_attribs.width;
	app->screen_h = app->x_attribs.height;
	...
	SDL_Surface *temp = SDL_CreateRGBSurfaceWithFormatFrom(
		app->image->data, app->screen_w, app->screen_h, 32,
	...
	while (XPending(app->x_display)) {
		...
		switch (x_evt->type) {
			...
			case SelectionRequest: {
				XSelectionRequestEvent *sev = &x_evt->xselectionrequest;
